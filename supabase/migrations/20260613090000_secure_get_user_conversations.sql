-- S3 (BLOCKER): get_user_conversations(p_user_id) was SECURITY DEFINER and
-- filtered only on the caller-supplied p_user_id, never comparing it to
-- auth.uid(). Any authenticated (or even anon) caller could read ANY user's
-- entire inbox (DM partner names/avatars, last-message previews, unread counts).
--
-- Fix: add an authorization guard so the caller may only read their own
-- conversations. Body is otherwise the live definition from
-- 20250903074417_fix_ambiguous_conversation_id.sql. Also pins search_path (S9).
